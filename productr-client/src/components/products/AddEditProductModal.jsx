// src/components/products/AddEditProductModal.jsx

import React, { useState, useEffect } from "react";
import "../../styles/Modal.css";

const PRODUCT_TYPES = [
  "Foods", "Electronics", "Clothes", "Beauty Products", "Others",
];

const AddEditProductModal = ({ onClose, onSubmit, editProduct = null }) => {
  ...
  useEffect(() => {
    if (editProduct) {
      setFormData({
        ...
        exchangeEligible: editProduct.exchangeEligible || "Yes",
      });
      setImages(
        (editProduct.images || []).map((src) => ({
          preview: src,
          existing: true,
        }))
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const product = {
        ...formData,
        images: existingUrls,   // existing Cloudinary URLs
        imageFiles: newFiles,   // new files to upload
      };
